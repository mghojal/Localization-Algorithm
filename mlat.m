classdef mlat
  ...
  methods (Access = public ,Static)
      %% main function
      function position = do_main(Algorithm, anchor_Location, ranges_in, prePos, varargin)
          ...
          switch Algorithm
              case 'Gradient_Descent'
                bounds_in = mlat.do_findBounds(anchor_Location, prePos);
                position = mlat.do_gdescent(anchor_Location, ranges_in, 'bounds', bounds_in);
              case 'Recursive_Least_square'
...
%           end
 ...
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%   
    function bounds_in = do_findBounds(anchors_in, prePos)
        alpha = 0.35;
        if isempty(prePos) 
            for i = 1:size(anchors_in, 2)
                bounds_in(1, i) = min(anchors_in(:, i)); % minimum boundary of ith axis
                bounds_in(2, i) = max(anchors_in(:, i)); % maximum boundary of ith axis
            end
            % hard coded minimum height (0 m) of search boundary
            bounds_in(1, end) = 0;
        else
            bounds_in(1,:) = prePos-[alpha alpha alpha];
            bounds_in(2,:) = prePos+[alpha alpha alpha];
        end
    end
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%   
